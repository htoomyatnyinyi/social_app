...
export default function MeditationTimer() {
  ...
  const { data: stats } = useGetMeditationStatsQuery(undefined, {
    skip: !theme,
  });
  ...
  const toggleTimer = useCallback(() => {
    ...
    // Call API only when user actually taps the button
  ...
  // 2. Timer Logic (FIXED: No 'timeLeft' in dependencies)
  useEffect(() => {
    if (isActive) {
      ...
      timerRef.current = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            ...
            if (repeat) {
              return intervalTime * 60; // Loop back
            } else {
              setIsActive(false);
              updateStatus({ isMeditating: false }).catch(() => {});
              return 0;
  ...
  }, [isActive, repeat, intervalTime, playBell, recordSession, updateStatus]);
    ...
    <View className={`flex-1 ${isDark ? "bg-[#0F172A]" : "bg-[#F8FAFC]"}`}>
      ...
      >
        ...
        <View className="flex-1 items-center justify-center pb-10">
          <View
            className={`flex-row items-center px-4 py-2 rounded-full mb-10 border ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-100"}`}
          >
            <View
          ...
          <View className="w-[300px] h-[300px] items-center justify-center mb-10">
            ...
            />
            <View
              className={`w-[250px] h-[250px] rounded-full items-center justify-center shadow-2xl border-2 ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-gray-50"}`}
            >
              <Text
                className={`text-6xl font-extralight tracking-tighter ${isDark ? "text-white" : "text-slate-900"}`}
              >
                {formatTime(timeLeft)}
              </Text>
              <Text
                style={{ color: accentColor }}
                className="text-[11px] font-black tracking-[4px] uppercase mt-2"
              >
          ...
          <View className="w-full px-10">
            <View
              className={`flex-row p-5 rounded-[32px] mb-8 border justify-around ${isDark ? "bg-slate-800/50 border-slate-700" : "bg-white border-gray-100 shadow-sm"}`}
            >
            ...
            <View
              className={`flex-row items-center justify-between p-2 rounded-3xl mb-10 border ${isDark ? "bg-slate-800 border-slate-700" : "bg-gray-50 border-gray-100"}`}
            >
              <View className="flex-row space-x-2">
                {[30, 60].map((t) => (
                  ...
                  >
                    <Text
                      className={`font-bold ${intervalTime === t ? (isDark ? "text-white" : "text-slate-900") : "text-slate-400"}`}
                    >
                      {t}m
                    </Text>
                  </TouchableOpacity>
              ...
              >
                <Text
                  className={`text-[9px] font-black uppercase mr-2 ${repeat ? "" : "text-slate-500"}`}
                  style={repeat ? { color: accentColor } : {}}
                >
