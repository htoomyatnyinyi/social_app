...
// Extended FAQ data with content
...
> = {
  ...
  guidelines: {
    ...
    content:
      "Our community guidelines ensure a safe environment:\n\n✅ DO:\n• Be respectful to others\n• Share authentic content\n• Report inappropriate behavior\n\n❌ DON'T:\n• Harass or bully others\n• Share explicit content\n• Impersonate others\n• Spam or scam\n\nViolations may result in account suspension.",
    category: "safety",
  },
  "change-username": {
...
// Category-specific FAQs mapping
const CATEGORY_FAQS: Record<string, string[]> = {
  account: ["delete-account", "password-recovery", "login-help"],
  safety: ["guidelines"],
  features: ["change-username"],
  privacy: [],
};
...
export default function HelpCenterScreen() {
  ...
  const helpCategories: Category[] = [
    {
      ...
      title: "Account",
      icon: "person-outline",
      description: "Verification & Login",
      color: "#0EA5E9",
    },
    {
      id: "safety",
  ...
  const filteredFAQs = useMemo(() => {
    ...
    if (selectedCategory) {
      const categorySlugs = CATEGORY_FAQS[selectedCategory] || [];
      faqs = faqs.filter((faq) => categorySlugs.includes(faq.slug));
    }
  ...
  const handleContactSupport = () => {
    ...
    Alert.alert("Contact Support", "Choose your preferred contact method", [
      {
        text: "Email Support",
        onPress: () => {
  ...
  const handleFAQPress = (faq: FAQ) => {
    ...
    router.push({
      pathname: "/settings/help/article/[slug]",
      params: { slug: faq.slug, title: faq.title },
    });
  };
  ...
  return (
    <View className={`flex-1 ${isDark ? "bg-[#0F172A]" : "bg-[#F8FAFC]"}`}>
        ...
        <View className="flex-row items-center justify-between">
          ...
          {(selectedCategory || searchQuery) && (
            <TouchableOpacity
              ...
              className="px-3 py-2 rounded-full bg-opacity-20"
      ...
      >
        {/* Search Hero Section */}
        <View
          className={`p-8 ${isDark ? "bg-slate-900/50" : "bg-white"} border-b ${isDark ? "border-slate-800" : "border-gray-100"}`}
        >
          <Text
            className={`text-3xl font-black leading-tight tracking-tight mb-2 ${isDark ? "text-white" : "text-gray-900"}`}
          >
            How can we{"\n"}help you?
          </Text>
          <Text
            className={`text-[14px] font-medium mb-6 ${isDark ? "text-slate-400" : "text-gray-500"}`}
          >
        ...
        <View className="p-5 flex-row flex-wrap justify-between mt-4">
          {helpCategories.map((item) => (
            <TouchableOpacity
              ...
              className={`w-[48%] p-5 rounded-[32px] mb-4 border shadow-sm ${
              ...
              style={{
                borderColor:
                  selectedCategory === item.id ? item.color : undefined,
              }}
            >
              <View
              ...
              <Text
                className={`font-black text-[15px] tracking-tight ${isDark ? "text-white" : "text-gray-900"}`}
              >
                {item.title}
              </Text>
              <Text
                className={`text-[10px] font-bold uppercase tracking-wide mt-1 ${isDark ? "text-slate-500" : "text-gray-400"}`}
              >
                {item.description}
              </Text>
              {selectedCategory === item.id && (
        ...
        <View className="px-5 mt-4">
          <View className="flex-row justify-between items-center mb-4">
            ...
            {filteredFAQs.length > 0 && !searchQuery && !selectedCategory && (
              <TouchableOpacity
                onPress={() => router.push("/settings/help/all-faqs")}
              >
                <Text
                  style={{ color: accentColor }}
                  className="text-xs font-bold"
          ...
          >
            {filteredFAQs.length > 0 ? (
              ...
              <View className="p-10 items-center">
                ...
                />
                <Text
                  className={`mt-2 font-bold text-center ${isDark ? "text-slate-500" : "text-gray-400"}`}
                >
                  {searchQuery
        ...
        <View className="px-5 mt-10">
          ...
          >
            ...
            <View className="bg-white mt-6 py-4 rounded-2xl items-center">
              <Text
                style={{ color: accentColor }}
                className="font-black uppercase tracking-widest text-[13px]"
              >
                Contact Support
              </Text>
            </View>
        ...
        <View className="px-5 mt-6 flex-row justify-between">
          <TouchableOpacity
            onPress={() => {
              Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              router.push("/settings/help/report-bug");
            }}
            className={`flex-1 mr-2 p-4 rounded-2xl flex-row items-center justify-center border ${
          ...
          >
            ...
            />
            <Text
              className={`ml-2 font-bold ${isDark ? "text-slate-300" : "text-gray-700"}`}
            >
              Report Bug
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            className={`flex-1 ml-2 p-4 rounded-2xl flex-row items-center justify-center border ${
              isDark ? "border-slate-700" : "border-gray-200"
            }`}
          >
            <Ionicons
        ...
        <View className="mt-12 items-center opacity-30">
          ...
          <Text
            className={`text-[9px] mt-2 font-bold uppercase ${isDark ? "text-slate-400" : "text-gray-500"}`}
          >
            Support Engine v2.4.1009
          </Text>
        </View>
      </ScrollView>
    </View>
...
const FAQItem = ({
  ...
  >
    <Text
      className={`text-[15px] font-bold tracking-tight flex-1 pr-4 ${isDark ? "text-slate-200" : "text-gray-700"}`}
    >
      {title}
    </Text>
    <Ionicons
...
//         <View className="px-5 mt-4">
//           <Text
//             className={`px-1 mb-4 text-[10px] font-black uppercase tracking-[2px] ${isDark ? "text-slate-500" : "text-gray-400"}`}
//           >
...
//                 />
//                 <Text
//                   className={`mt-2 font-bold ${isDark ? "text-slate-500" : "text-gray-400"}`}
//                 >
//                   No results found
//                 </Text>
//               </View>
