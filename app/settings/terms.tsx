  ...
  const sections = [
    {
      ...
      content:
        "You agree to use the service in compliance with all applicable laws and regulations. You are responsible for your content and conduct within the platform.",
      icon: "rocket-outline",
    },
    {
      id: "02",
      title: "User Account",
      content:
        "You are responsible for safeguarding your account credentials. Any activity performed under your account is your sole responsibility.",
    ...
    },
    {
      ...
      content:
        "We respect intellectual property. You must not post content that violates copyright, trademark, or any local decency laws. We reserve the right to remove non-compliant content.",
      icon: "layers-outline",
    },
    {
      id: "04",
      title: "Termination",
      content:
        "We reserve the right to suspend or terminate your account at our sole discretion, without notice, for conduct that we believe violates these Terms or harms other users.",
  ...
  return (
    <View className={`flex-1 ${isDark ? "bg-[#0F172A]" : "bg-[#F8FAFC]"}`}>
      ...
      >
        {/* Intro Tag */}
        <View
          className={`mb-8 p-6 rounded-[32px] border ${isDark ? "bg-slate-800/30 border-slate-800" : "bg-white border-gray-100 shadow-sm shadow-gray-100"}`}
        >
          <Text
            className={`text-[11px] font-black uppercase tracking-[2px] mb-2 ${isDark ? "text-slate-500" : "text-gray-400"}`}
          >
            Last Updated:{" "}
          ...
          <Text
            className={`text-base font-medium leading-[24px] ${isDark ? "text-slate-300" : "text-gray-600"}`}
          >
            By accessing this application, you enter a legally binding contract.
          ...
          <View key={index} className="mb-6">
            <View className="flex-row items-center mb-3 px-1">
              <Text
                ...
                className="font-black text-lg tracking-tighter mr-2"
              >
                {item.id}
              </Text>
              <View
                className={`h-[1px] flex-1 ${isDark ? "bg-slate-800" : "bg-gray-100"}`}
              />
            </View>
            ...
            >
              <View className="flex-row items-center mb-3">
                <View
                  className={`w-8 h-8 rounded-xl items-center justify-center mr-3 ${isDark ? "bg-slate-700" : "bg-gray-50"}`}
                >
                  <Ionicons
              ...
              <Text
                className={`text-[14px] leading-[22px] font-medium ${isDark ? "text-slate-400" : "text-gray-500"}`}
              >
                {item.content}
              </Text>
            </View>
        ...
        <View className="mt-4 px-2">
          <Text
            className={`text-[12px] text-center font-bold italic ${isDark ? "text-slate-600" : "text-gray-400"}`}
          >
            Continued use of the app constitutes acceptance of any future
