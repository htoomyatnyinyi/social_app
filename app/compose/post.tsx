...
export default function ComposePostScreen() {
  ...
  // MCP Tool Mutation
  const [callMcpTool, { isLoading: isMcpLoading }] = useCallMcpToolMutation();

  const isLoading =
    isCreating || isReposting || isReplying || isFetchingLocation || isMcpLoading;
  ...
  const handleEnhanceDraft = async () => {
    ...
    try {
      ...
      Alert.alert("AI Error", "Could not enhance draft right now.");
    }
  };
  ...
  const handleSuggestHashtags = async () => {
    ...
    try {
      ...
      Alert.alert("AI Error", "Could not fetch hashtags right now.");
    }
  };
    ...
    <View style={{ flex: 1, backgroundColor: isDark ? "#0F172A" : "#F8FAFC" }}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          className={`px-8 py-2 rounded-2xl ${(!content.trim() && images.length === 0) || isLoading
        ...
        >
          {isLoading && !isMcpLoading ? (
            ...
            <Text className={`font-black text-xs ${(!content.trim() && images.length === 0) || isLoading ? "text-gray-400" : "text-white"}`}>POST</Text>
          )}
        </TouchableOpacity>
      </BlurView>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          >
            <View className="px-5 py-3 flex-row items-center justify-between">
              <View className="flex-row items-center">
                <TouchableOpacity
                  ...
                  className="w-10 h-10 items-center justify-center rounded-xl bg-gray-50/10 dark:bg-slate-800/50 mr-2"
                >
                ...
                {/* --- AI TOOLS --- */}
                <View className="w-[1px] h-6 bg-gray-300/30 dark:bg-gray-700/50 mx-1 mr-3" />

                <TouchableOpacity
                  onPress={handleEnhanceDraft}
                  disabled={!content.trim() || isMcpLoading}
                  className={`w-10 h-10 items-center justify-center rounded-xl mr-2 ${!content.trim() || isMcpLoading
                    ? "bg-gray-50/10 dark:bg-slate-800/50"
                ...
                >
                  {isMcpLoading ? (
                    ...
                    <Ionicons
                      name="sparkles"
                      size={20}
                      color={content.trim() ? "#818CF8" : "#64748B"}
                    />
                  )}
                </TouchableOpacity>

                <TouchableOpacity
                  onPress={handleSuggestHashtags}
                  disabled={!content.trim() || isMcpLoading}
                  className={`w-10 h-10 items-center justify-center rounded-xl ${!content.trim() || isMcpLoading
                    ? "bg-gray-50/10 dark:bg-slate-800/50"
                    : "bg-indigo-50 dark:bg-indigo-900/30"
                    }`}
                >
                  <Ionicons
                    ...
                    size={20}
                    color={content.trim() && !isMcpLoading ? "#818CF8" : "#64748B"}
                  />
                </TouchableOpacity>
              </View>

              <View className="bg-gray-100/10 dark:bg-slate-800/50 px-3 py-2 rounded-lg">
            ...
            {/* Adjust safe area padding based on keyboard */}
            {!isKeyboardOpen && (
